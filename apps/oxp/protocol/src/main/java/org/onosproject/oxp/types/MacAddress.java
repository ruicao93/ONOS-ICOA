 ...
 */
public class MacAddress implements OXPValueType<MacAddress> {
    ...
    /**
     * Creates a {@link MacAddress} from a {@link DomainId}. This factory
     * method assumes that the first two bytes of the {@link DomainId} are 0 bytes.
     * @param dmid the {@link DomainId} to create the {@link MacAddress} from
     * @return a {@link MacAddress} derived from the supplied {@link DomainId}
     */
    public static MacAddress of(@Nonnull DomainId dmid) {
        Preconditions.checkNotNull(dmid, "dmid must not be null");

        long raw = dmid.getLong();
    ...
    public static MacAddress read6Bytes(ChannelBuffer c) throws OXPParseError {
